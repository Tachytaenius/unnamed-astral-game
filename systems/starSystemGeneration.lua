...
local function generateSystem(parent, info, depth, ownI, state, graphicsObjects)
	...
	for i = 1, numBodies do
		local gaseousParent = parent and parent.celestialBody.type == "gaseous"
		local ringsProbability = 0.4 / (1 + 5 * ringsCount) -- The more rings we have the less likely we are to get more
		local isRingSystem = gaseousParent and love.math.random() < ringsProbability

		if isRingSystem then
			ringsCount = ringsCount + 1
			if not parent.ringSystemsList then
				parent:give("ringSystemsList")
			end

			local function generateColour()
				return {
					love.math.random(),
					love.math.random(),
					love.math.random()
			...
			local ring = concord.entity()
			ring:give("ringSystem", parent,

				"icy",

				-- The distances the rings exist at should be much less than the distances moons tend do...?
				parent.celestialRadius.value * ((0.4 + i) + util.randomRange(-0.125, 0.125)), -- i starts at 1
				parent.celestialRadius.value * util.randomRange(0.2, 0.4),
				...
				util.randomRange(1, 5),
				util.randomRange(1, 5),
				util.randomRange(6, 12),
				love.math.random() < 0.5 and util.randomRange(0.2, 0.6) or 0
			)

			parent.ringSystemsList.value[#parent.ringSystemsList.value + 1] = ring
			state.ecs:addEntity(ring)
		else
			...
			if not parent then
				...
				-- The distances moons exist at should be much greater than those that rings do...?
